    ...
    until answer == 99
      ...
      if answer == 1
          ...
          apartments.each do |apt|
            puts "(ID) #{apt[:id]}   (Address) #{apt[:address]}   (Rent) #{apt[:monthly_rent]}   (Square Feet) #{apt[:square_feet]}" #id,address,monthly_rent,square_feet
          end

      elsif answer == 2

          tenants.each do |apt| # tenants: id,name,age,apartment_id
            puts "(ID) #{apt[:id]}   (Name) #{apt[:name]}    (Age) #{apt[:age]}    (AptID) #{apt[:apartment_id]}" #id,address,monthly_rent,square_feet
          end

      elsif answer == 3

          aptTenants(apartments,tenants)

      elsif answer == 4

          tenantHome(apartments,tenants)
          ...
          hash = addApt(id,address,monthly_rent,square_feet)
          ...
          hash = addTenant(id,name,age,apartment_id)
          tenants.push(hash)
          puts " "
